...
p7.createSignedData = function () {
  ...
  function addDigestAlgorithmIds() {
    ...
    for (var i = 0; i < msg.signers.length; ++i) {
      ...
      if (!(oid in mds)) {
        // content digest
        if (oid === forge.pki.oids.sha256WithRSAEncryption) {
          mds[oid] = forge.md[forge.pki.oids.sha256].create();
        } else {
      ...
      if (signer.authenticatedAttributes.length === 0) {
        ...
        if (oid === forge.pki.oids.sha256WithRSAEncryption) {
          signer.md = forge.md[forge.pki.oids.sha256].create();
        } else {
...
function _attributeToAsn1(attr) {
  ...
  if (attr.type === forge.pki.oids.contentType) {
    ...
    value = asn1.create(asn1.Class.UNIVERSAL, asn1.Type.SEQUENCE, true, [
      attr.value,
      // asn1.create(asn1.Class.UNIVERSAL, asn1.Type.SEQUENCE, true, [attr.value]),
    ]);
  }
